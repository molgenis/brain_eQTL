...
class main():
    def __init__(self):
        ...
        self.remove_multiallelic = getattr(arguments, 'no_keep_multialleic')
        self.remove_non_pass_snv = getattr(arguments, 'no_keep_non_pass_snv')
        self.remove_non_pass_indel = getattr(arguments, 'no_keep_non_pass_indel')
        self.filer_low_complexity = getattr(arguments, 'no_keep_low_complexity')
    ...
    def create_argument_parser():
        ...
        parser.add_argument("--no_keep_multialleic",
                            action='store_false',
                            help="Add this flag to remove multiallelic "
                                 "variants. Default: False.")
        parser.add_argument("--no_keep_non_pass_snv",
                            action='store_false',
                            help="Add this flag to remove SNV's without PASS "
                                 "label. Default: False.")
        parser.add_argument("--no_keep_non_pass_indel",
                            action='store_false',
                            help="Add this flag to remove indel's without PASS "
                                 "label. Default: False.")
        parser.add_argument("--no_keep_low_complexity",
                            action='store_false',
    ...
    def parse_line(self, line_number, line, sample_mask, sample_male_mask):
        ...
        for i in range(9, len(elems)):
            ...
            if sampledata is not None:
                ...
                if self.ignore_homref_stats and dosage == 0:
