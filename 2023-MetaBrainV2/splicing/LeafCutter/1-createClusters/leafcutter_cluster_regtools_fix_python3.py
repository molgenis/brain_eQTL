...
def pool_junc_reads(flist, options):
    ...
    outFile = "%s/%s_pooled.gz"%(rundir,outPrefix)
    ...
    for libl in flist:
        ...
        for ln in getrfh(lib):
            ...
            lctr += 1
        print("Lines parsed: {}".format(lctr))
    fout = getwfh(outFile)
    Ncluster = 0
...
def sort_junctions(libl, options):
    ...
    if options.cluster == None:
        refined_cluster = "%s/%s_refined.gz"%(rundir,outPrefix)
    else:
    ...
    for ln in getrfh(refined_cluster):
        chrom = ln.split()[0]
        cluN += 1
    ...
    fout_runlibs = getwfh(runName+"_sortedlibs.gz")
...
def refine_clusters(options):
    ...
    inFile = "%s/%s_pooled.gz"%(rundir,outPrefix)
    outFile = "%s/%s_refined.gz"%(rundir,outPrefix)
    fout = getwfh(outFile)
...
def merge_junctions(options):    
    ...
    flist = "%s/%s_sortedlibs.gz"%(rundir, outPrefix)
    
    lsts = []
    for ln in getrfh(flist):
    ...
    while len(lsts) > 1:    
        ...
        for i in range(0,int( (len(lsts)/N)+1) ):
            lst = lsts[N*i:N*(i+1)]
            if len(lst) > 0:
                clst.append(lst)
        lsts = []
...
def get_numers(options):
    ...
    input_file=getrfh(fname)
    fout = getwfh(fnameout)
    first_line=True
